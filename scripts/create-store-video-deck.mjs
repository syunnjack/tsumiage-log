...
const queue = JSON.parse(await fs.readFile(path.join(root, "app/data/store-videos.json"), "utf8"))
const item = queue.videos.find((video) => video.slug === slug)
if (!item) throw new Error(`Store video entry not found: ${slug}`)
...
for (const [index, definition] of item.slides.entries()) {
  ...
  if (index === 0) {
    addText(
      ...
      { fontSize: 52, bold: true, color: palette.ink },
      "title",
    )
    addText(
      slide,
      definition.body,
      { left: 76, top: 380, width: 940, height: 82 },
    ...
    const freeCount = item.freePreviewSlideCount ?? 0
    if (index >= freeCount) {
      addText(
        slide,
        "PPV限定パート",
        { left: 900, top: 40, width: 220, height: 30 },
        { fontSize: 14, bold: true, color: palette.accent, alignment: "right" },
        `ppv-badge-${index}`,
      )
    }
  }
